...
// РАЗБОР КОМАНДЫ ПОСЛЕ ПРОВЕРКИ СТРУКТУРЫ
   ...
   public:
    ...
    void set_visual_temperature_step_override(float val){
        this->temperature_step=val;    
        _traits.set_visual_target_temperature_step(val);
        _traits.set_visual_current_temperature_step(val);
    }
